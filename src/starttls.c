...
bool starttls_handshake (int fd_old,
			dercrs client_hostname, dercrs server_hostname,
...
bool starttls_remote_hostname_check_certificate (struct dercrs hostname,
...
bool starttls_local_realm_check_certificate (struct dercrs localrealm,
			struct starttls_data *tlsdata,
 ...
 * or false with errno set otherwise.
 */
bool startls_remote_realm_check_certificate (struct dercrs remoterealm,
			struct starttls_data *tlsdata,
			starttls_cb_test_t cb, void *cbdata) {
	//TODO// Implement this with a new async call to the TLS Pool
	cb (cbdata, true);
 ...
 * or false with errno set otherwise.
 */
bool starttls_export_key (dercrs label, dercrs opt_ctxval,
